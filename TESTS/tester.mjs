...
const loadPuppeteer = async () => {
  if (!puppeteer) {
    try {
      ...
      fatal(
        'Puppeteer is required for DOM exercises.\n' +
        'Install it with:\n\n' +
...
// ✅ ALL tests live here now
const testsDir = joinPath(root, 'tests-files')
...
const testNode = async () => {
  ...
  return {
    path,
    // ✅ NEW: mjs tests live in tests/js/
    url: joinPath(testsDir, `${name}_test.mjs`),
...
const main = async () => {
  const { test, mode } = await any([
    // ✅ NEW: JSON tests from tests/json/
    readTest(joinPath(testsDir, `${name}.json`)),
    readTest(joinPath(testsDir, `${name}_test.js`)),
    readTest(joinPath(testsDir, `${name}_test.mjs`)),
